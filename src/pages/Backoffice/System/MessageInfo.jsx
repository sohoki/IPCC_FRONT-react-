...
const MessageFormModal = lazy(() => import('@/pages/Backoffice/System/components/MessageFormModal.jsx'));

const INITIAL_FORM = {
    mode: 'Ins',
    gubun: 'queue',    // queue | exchange
    queue: '',         // Queue 이름
    processNm: '',     // Exchange 이름
    routingKey: 'Y',   // 내구성(Durable)
    autoDel: 'N',      // 자동삭제
    type: 'N',         // queue: 'Y'|'N'(autoDelete) / exchange: direct|fanout|topic|headers
    useAt: 'Y',
    idCheck: 'N',
};
...
const MessageInfo = () => {
    ...
    const handleDeleteQueue = useCallback(async ({ queueNm }) => {
        if (!queueNm) return;

        const ok = await Swal.fire({
            icon: 'question',
            title: 'Queue 삭제',
            html: `<b>${queueNm}</b> 삭제 하시겠습니까?`,
        ...
        try {
            ...
            if (json?.resultCodeInfo === 'SUCCESS') {
                await Swal.fire({ icon: 'success', title: '성공', text: json?.resultMessage || '성공적으로 삭제되었습니다.' });
                handleSearch(1);
            } else {
                await Swal.fire({ icon: 'warning', title: '경고', text: json?.resultMessage || '삭제에 실패했습니다.' });
            }
        } catch (e) {
    ...
    const { handleSubmit } = useCommonSubmit({
        ...
        type: 'json',
        checkField: [],
        confirmMessage: form.gubun === 'exchange'
            ? `Exchange [${form.processNm}]를 저장하시겠습니까?`
            : `Queue [${form.queue}]를 저장하시겠습니까?`,
        gridApiRef,
        setModalOpen,
        URL: URL.QUEUE_CREATE,
        reloadFunction: () => onSearch(1),
    });

    const columnDefs = useMemo(() => [
        { headerName: 'Queue', field: 'queueNm', flex: 1, sortable: true },
        {
            headerName: 'Queue Duration',
            field: 'queueDuration',
            width: 120,
            cellRenderer: (p) => {
                const v = p.value;
                return (
                    <span className={`badge rounded-pill ${v === 'Y' ? 'bg-success' : 'bg-secondary'} px-3 py-2`}
        ...
        {
            headerName: '자동삭제',
            field: 'queueAutodelete',
            width: 110,
            cellRenderer: (p) => {
                const v = p.value;
                return (
                    <span className={`badge rounded-pill ${v === 'Y' ? 'bg-warning text-dark' : 'bg-secondary'} px-3 py-2`}
                        style={{ fontSize: '0.82rem' }}>
                        {v === 'Y' ? '자동' : '수동'}
                    </span>
                );
        ...
        { headerName: 'System Code', field: 'systemCode', width: 150 },
        { headerName: 'Last Updated User', field: 'lastUpdusrId', width: 120 },
        { headerName: 'Last Updated Time', field: 'lastUpdtPnttm', width: 170 },
        {
            headerName: '삭제',
            width: 80,
            sortable: false,
            ...
            cellRenderer: (p) => (
                <button
                    ...
                    onClick={() => handleDeleteQueue({ queueNm: p.data?.queueNm })}
                >
                    삭제
                </button>
            ),
    ...
    return (
        <div className="row g-0 main-contents">
            <div className="col-12 content-header">
                <div className="content-header__title">메세지 QUEUE 관리</div>
                <div className="content-header__breadcrumb">
                    <ol className="breadcrumb">
                        <li className="breadcrumb-item">시스템 관리</li>
                        <li className="breadcrumb-item">메세지 QUEUE 관리</li>
                    </ol>
                </div>
            </div>
