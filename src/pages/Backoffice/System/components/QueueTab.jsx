...
const QueueFormModal = lazy(() => import('./QueueFormModal.jsx'));
...
const QueueTab = () => {
  ...
  const fetchData = useCallback(async (query) => {
    const res = await fnAjaxFetch({ url: URL.QUEUE_LIST, method: 'POST', data: query });
    const data = res?.data;
    return {
  ...
  const { handleSubmit } = useCommonSubmit({
    ...
    confirmMessage: `Queue [${form.queue}]를 저장하시겠습니까?`,
    gridApiRef, setModalOpen, URL: URL.QUEUE_CREATE,
    reloadFunction: () => onSearch(1),
  });

  const columnDefs = useMemo(() => [
    { headerName: 'Queue 이름', field: 'queueNm', flex: 1 },
    {
      headerName: '내구성', field: 'queueDuration', width: 110,
      cellRenderer: (p) => (
    ...
    {
      headerName: '자동삭제', field: 'queueAutodelete', width: 110,
      cellRenderer: (p) => (
    ...
    { headerName: 'Queue 유형', field: 'codeNm', width: 110 },
    ...
    { headerName: '최종 수정일', field: 'lastUpdtPnttm', width: 150 },
    {
      ...
      cellRenderer: (p) => (
        <button className="btn btn-outline-danger btn-outline__gray btn-delete"
          onClick={() => handleDelete({ code: p.data?.queueNm, name: p.data?.queueNm })}>삭제</button>
      ),
    },
  ], [handleDelete]);
  ...
  return (
    <>
      <div className="col-12 content-search">
        <div className="row g-0 w-100 justify-content-between">
          <div className="col-auto content-search__option">
            <input type="text" name="searchKeyword" placeholder="Queue 이름으로 검색"
      ...
      <QueueFormModal open={modalOpen} onClose={() => setModalOpen(false)}
