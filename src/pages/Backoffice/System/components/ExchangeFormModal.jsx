...
const ExchangeFormModal = ({ open, onClose, form, setForm, onSubmit }) => {
  const { renderRadioGroup } = useRadioGroup(form, setForm);
  const { handleIdCheck } = useIdCheck(URL.EXCHANGE_ID_CHECK, 'Exchange 이름');

  const onCheckId = useCallback(async () => {
    await handleIdCheck(form.processNm, setForm, { gubun: 'exchange', processNm: form.processNm });
  }, [form.processNm, setForm, handleIdCheck]);
  ...
  const handleValidatedSubmit = useCallback(async () => {
    ...
    if (form.idCheck !== 'Y') {
      await Swal.fire({ icon: 'warning', text: 'Exchange 이름 중복확인을 해주세요.' });
      return;
    }
    onSubmit();
  }, [form, onSubmit]);
          ...
          <div className="modal-content">
            <div className="modal-header">
              <div className="modal-title">
                <h2 className="modal-title__title">Exchange 등록</h2>
              </div>
