...
const CtiDnInfo = () => {
    ...
    const handleDelete = useCallback(async (dn, centerId, tenantId, dnmajorId) => {
        const first = await Swal.fire({
            icon: 'question',
            title: 'DN ??†ú',
            html: `<b>${dn}</b> Î•??? ??†ú ?òÏãúÍ≤†Ïäµ?àÍπå?`,
        ...
        const second = await Swal.fire({
            icon: 'warning',
            title: 'DN ??†ú ?ïÏù∏',
            html: `<b>${dn}</b> Î•??? ??†ú?òÏãúÎ©??úÏä§?úÏóê ?ÅÌñ•???àÏùÑ ???àÏäµ?àÎã§.<br>?ïÎßêÎ°???†ú?òÏãúÍ≤†Ïäµ?àÍπå?`,
            showCancelButton: true,
            confirmButtonText: '??,
            cancelButtonText: '?ÑÎãà??,
            focusCancel: true,
    ...
    const columnDefs = useMemo(() => [
        { headerName: '?ºÌÑ∞Î™?, field: 'centerName', width: 110 },
        { headerName: '?åÎÑå?∏Î™Ö', field: 'tenantName', width: 110 },
        { headerName: 'DN ?ÄÎ∂ÑÎ•ò', field: 'dnmajorId', width: 100 },
        { headerName: 'DN ?åÎ∂ÑÎ•?, field: 'dnsubId', width: 100 },
        { headerName: 'DN', field: 'dn', width: 110 },
        { headerName: 'MediaId', field: 'mediaId', width: 90 },
        { headerName: 'Î∂ÄÎØ∏Îîî??, field: 'submediaId', width: 80 },
        { headerName: 'Î™®Îç∏Î™?, field: 'dnModelname', width: 110 },
        { headerName: 'ServiceDesc', field: 'dnServicedesc', width: 110 },
        { headerName: 'Ï¢ÖÎ•ò', field: 'dnKind', width: 80 },
        { headerName: '?Ä??, field: 'dnType', width: 80 },
        { headerName: 'IP', field: 'dnIp', width: 100 },
        { headerName: 'Í∞êÏ≤≠?¨Î?', field: 'observerFlag', width: 90 },
        { headerName: 'Í∞êÏãú', field: 'monitorFlag', width: 80 },
    ...
    return (
        <div className="row g-0 main-contents">
            <div className="col-12 content-header">
                <div className="content-header__title">CTI DN ?ÑÌô©</div>
                <div className="content-header__breadcrumb">
                    <ol className="breadcrumb">
                        <li className="breadcrumb-item">?∏ÌîÑ??Í¥ÄÎ¶?/li>
                        <li className="breadcrumb-item">CTI DN ?ÑÌô©</li>
                    </ol>
                </div>
            </div>
