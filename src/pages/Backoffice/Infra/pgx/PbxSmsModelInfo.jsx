...
const PbxSmsModelInfo = () => {
    ...
    // ?�택??SMS MODEL ?�정?�로 Avaya SMS ?� ?�신?�여 ?�이??조회
    const handleAvayaQuery = useCallback(async (rowData) => {
        if (rowData?.smsUseyn !== 'Y') {
            await Swal.fire({ icon: 'warning', text: '?�용 ?�함 ?�태??SMS MODEL ?�니??' });
            return;
        }
        try {
            const res = await fnAjaxFetch({
            ...
            if (json?.resultCodeInfo === 'SUCCESS') {
                ...
                await Swal.fire({ icon: 'error', text: json?.resultMessage || 'Avaya 조회???�패?�습?�다.' });
            }
        } catch (e) {
    ...
    const handleDelete = useCallback(async (notiSeq) => {
        ...
        const second = await Swal.fire({
            icon: 'warning',
            title: 'Sms Model ??�� ?�인',
            html: `<b>${notiSeq}</b> �??? ??��?�시�??�스?�에 ?�향???�을 ???�습?�다.<br>?�말�???��?�시겠습?�까?`,
            showCancelButton: true,
            confirmButtonText: '??,
            cancelButtonText: '?�니??,
            focusCancel: true,
        ...
        try {
            ...
            if (json?.STATUS === 'SUCCESS' || json?.resultCodeInfo === 'SUCCESS') {
                await Swal.fire({ icon: 'success', text: json?.message || json?.MESSAGE || '??��?�었?�니??' });
                handleSearch(1);
            } else {
                await Swal.fire({ icon: 'error', text: json?.message || json?.MESSAGE || '??��???�패?�습?�다.' });
            }
        } catch (e) {
            await Swal.fire({ icon: 'error', text: e?.message || '처리 �??�류가 발생?�습?�다.' });
        }
    ...
    const columnDefs = useMemo(() => [
        ...
        { headerName: 'sms �?,           field: 'smsName',       flex: 1 },
        { headerName: '?�영 지??,         field: 'smsOperation',  flex: 1 },
        { headerName: 'sms ?�용 구분',    field: 'smsGubun',      width: 120 },
        { headerName: '?�용?�무',          field: 'smsUseyn',      width: 90 },
        { headerName: '?�성??,            field: 'frstRegistPnttm', width: 150 },
        {
