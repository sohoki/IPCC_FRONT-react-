...
const PbxSmsModelInfo = () => {
    ...
    // ?†ÌÉù??SMS MODEL ?§Ï†ï?ºÎ°ú Avaya SMS ?Ä ?µÏã†?òÏó¨ ?∞Ïù¥??Ï°∞Ìöå
    const handleAvayaQuery = useCallback(async (rowData) => {
        if (rowData?.smsUseyn !== 'Y') {
            await Swal.fire({ icon: 'warning', text: '?¨Ïö© ?àÌï® ?ÅÌÉú??SMS MODEL ?ÖÎãà??' });
            return;
        }
        try {
            const res = await fnAjaxFetch({
            ...
            if (json?.resultCodeInfo === 'SUCCESS') {
                ...
                await Swal.fire({
                    icon: 'success',
                    title: `${rowData.smsModel} Ï°∞Ìöå Í≤∞Í≥º`,
                    html: `<pre style="text-align:left;max-height:50vh;overflow:auto;font-size:12px;">${
                        pretty.replace(/&/g, '&amp;').replace(/</g, '&lt;')
                    }</pre>`,
                    width: 800,
                    confirmButtonText: '?ïÏù∏',
                });
            } else {
                await Swal.fire({ icon: 'error', text: json?.resultMessage || 'Avaya Ï°∞Ìöå???§Ìå®?àÏäµ?àÎã§.' });
            }
        } catch (e) {
    ...
    const handleDelete = useCallback(async (notiSeq) => {
        const first = await Swal.fire({
            icon: 'question',
            title: 'Sms Model ??†ú',
            html: `<b>${notiSeq}</b> Î•??? ??†ú ?òÏãúÍ≤†Ïäµ?àÍπå?`,
        ...
        const second = await Swal.fire({
            icon: 'warning',
            title: 'Sms Model ??†ú ?ïÏù∏',
            html: `<b>${notiSeq}</b> Î•??? ??†ú?òÏãúÎ©??úÏä§?úÏóê ?ÅÌñ•???àÏùÑ ???àÏäµ?àÎã§.<br>?ïÎßêÎ°???†ú?òÏãúÍ≤†Ïäµ?àÍπå?`,
            showCancelButton: true,
            confirmButtonText: '??,
            cancelButtonText: '?ÑÎãà??,
            focusCancel: true,
        ...
        try {
            ...
            if (json?.STATUS === 'SUCCESS' || json?.resultCodeInfo === 'SUCCESS') {
                await Swal.fire({ icon: 'success', text: json?.message || json?.MESSAGE || '??†ú?òÏóà?µÎãà??' });
                handleSearch(1);
            } else {
                await Swal.fire({ icon: 'error', text: json?.message || json?.MESSAGE || '??†ú???§Ìå®?àÏäµ?àÎã§.' });
            }
        } catch (e) {
            await Swal.fire({ icon: 'error', text: e?.message || 'Ï≤òÎ¶¨ Ï§??§Î•òÍ∞Ä Î∞úÏÉù?àÏäµ?àÎã§.' });
        }
    ...
    const columnDefs = useMemo(() => [
        ...
        { headerName: 'sms Î™?,           field: 'smsName',       flex: 1 },
        { headerName: '?¥ÏòÅ ÏßÄ??,         field: 'smsOperation',  flex: 1 },
        { headerName: 'sms ?¨Ïö© Íµ¨Î∂Ñ',    field: 'smsGubun',      width: 120 },
        { headerName: '?¨Ïö©?†Î¨¥',          field: 'smsUseyn',      width: 90 },
        { headerName: '?ùÏÑ±??,            field: 'frstRegistPnttm', width: 150 },
        {
            headerName: 'Avaya Ï°∞Ìöå', width: 100, sortable: false, filter: false,
            cellRenderer: (p) => (
                <button
