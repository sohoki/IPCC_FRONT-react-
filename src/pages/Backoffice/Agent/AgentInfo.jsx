...
const AgentFormModal = lazy(() => import('@/pages/Backoffice/Agent/components/AgentFormModal.jsx'));
const AgentPbxSearchModal = lazy(() => import('@/pages/Backoffice/Agent/components/AgentPbxSearchModal.jsx'));
const AgentExcelModal = lazy(() => import('@/pages/Backoffice/Agent/components/AgentExcelModal.jsx'));
...
const AgentInfo = () => {
    ...
    const [pbxSearchModalOpen, setPbxSearchModalOpen] = useState(false);
    ...
    const handleOpenFormModal = useCallback((loginId = null) => {
    ...
    const handleDelete = useCallback(async (loginId) => {
        const first = await Swal.fire({
            icon: 'question',
            title: 'Agent ??†ú',
            html: `<b>${loginId}</b> Î•??? ??†ú ?òÏãúÍ≤†Ïäµ?àÍπå?`,
        ...
        const second = await Swal.fire({
            icon: 'warning',
            title: 'Agent ??†ú ?ïÏù∏',
            html: `<b>${loginId}</b> Î•??? ??†ú?òÏãúÎ©??úÏä§?úÏóê ?ÅÌñ•???àÏùÑ ???àÏäµ?àÎã§.<br>?ïÎßêÎ°???†ú?òÏãúÍ≤†Ïäµ?àÍπå?`,
            showCancelButton: true,
            confirmButtonText: '??,
            cancelButtonText: '?ÑÎãà??,
            focusCancel: true,
        ...
        try {
            ...
            if (json?.STATUS === 'SUCCESS' || json?.resultCodeInfo === 'SUCCESS') {
                await Swal.fire({ icon: 'success', title: '??†ú', text: json?.MESSAGE || '??†ú?òÏóà?µÎãà??' });
                handleSearch(1);
            } else {
                await Swal.fire({ icon: 'error', title: '?§Î•ò', text: json?.MESSAGE || '??†ú???§Ìå®?àÏäµ?àÎã§.' });
                handleSearch(1);
            }
        } catch (e) {
    ...
    const columnDefs = useMemo(() => [
        { headerName: 'Î°úÍ∑∏?∏ID', field: 'loginId', width: 130 },
        { headerName: '?¥Î¶Ñ', field: 'name', width: 120 },
        { headerName: '?úÎÇòÎ¶¨Ïò§', field: 'sn', width: 120 },
        { headerName: 'Index', field: 'snIndex', width: 100 },
        { headerName: '?§ÌÇ¨Level', field: 'sr', width: 100 },
        { headerName: '?§ÌÇ¨Index', field: 'srIndex', width: 100 },
        { headerName: 'ÏµúÏ¢Ö?òÏ†ï??, field: 'agentUpdate', flex: 1 },
        {
            headerName: '?òÏ†ï',
            width: 70,
            sortable: false,
    ...
    return (
        <div className="row g-0 main-contents">
            <div className="col-12 content-header">
                <div className="content-header__title">?êÏù¥?ÑÌä∏ ?ÑÌô©</div>
                <div className="content-header__breadcrumb">
                    <ol className="breadcrumb">
                        <li className="breadcrumb-item">IPCC Í¥ÄÎ¶?/li>
                        <li className="breadcrumb-item">?êÏù¥?ÑÌä∏ ?ÑÌô©</li>
                    </ol>
                </div>
            </div>
            ...
            />
            <AgentPbxSearchModal
                open={pbxSearchModalOpen}
                onClose={() => setPbxSearchModalOpen(false)}
                onSuccess={() => { setPbxSearchModalOpen(false); onSearch(1); }}
            />
            <AgentExcelModal
                open={excelModalOpen}
