...
const AgentInfo = () => {
    ...
    const handleDelete = useCallback(async (loginId) => {
        ...
        const second = await Swal.fire({
            icon: 'warning',
            title: 'Agent ??�� ?�인',
            html: `<b>${loginId}</b> �??? ??��?�시�??�스?�에 ?�향???�을 ???�습?�다.<br>?�말�???��?�시겠습?�까?`,
            showCancelButton: true,
            confirmButtonText: '??,
            cancelButtonText: '?�니??,
            focusCancel: true,
        ...
        try {
            ...
            if (json?.STATUS === 'SUCCESS' || json?.resultCodeInfo === 'SUCCESS') {
                await Swal.fire({ icon: 'success', title: '??��', text: json?.MESSAGE || '??��?�었?�니??' });
                handleSearch(1);
            } else {
                await Swal.fire({ icon: 'error', title: '?�류', text: json?.MESSAGE || '??��???�패?�습?�다.' });
                handleSearch(1);
            }
        } catch (e) {
    ...
    const columnDefs = useMemo(() => [
        { headerName: '로그?�ID', field: 'loginId', width: 130 },
        { headerName: '?�름', field: 'name', width: 120 },
        { headerName: '?�나리오', field: 'sn', width: 120 },
        { headerName: 'Index', field: 'snIndex', width: 100 },
        { headerName: '?�킬Level', field: 'sr', width: 100 },
        { headerName: '?�킬Index', field: 'srIndex', width: 100 },
        { headerName: '최종?�정??, field: 'agentUpdate', flex: 1 },
        {
            headerName: '?�정',
            width: 70,
            sortable: false,
    ...
    return (
        <div className="row g-0 main-contents">
            <div className="col-12 content-header">
                <div className="content-header__title">?�이?�트 ?�황</div>
                <div className="content-header__breadcrumb">
                    <ol className="breadcrumb">
                        <li className="breadcrumb-item">IPCC 관�?/li>
                        <li className="breadcrumb-item">?�이?�트 ?�황</li>
                    </ol>
                </div>
            </div>
