...
const AgentPbxSearchModal = ({ open, onClose, onSuccess }) => {
    ...
    const handleSearch = useCallback(async () => {
        try {
            ...
            if (json?.STATUS === 'SUCCESS') {
                ...
                await Swal.fire({ icon: 'warning', title: '조회', text: json?.MESSAGE || '조회 실패했습니다.' });
            }
        } catch (e) {
            await Swal.fire({ icon: 'error', title: '오류', text: e?.message || '처리 도중 문제가 발생 하였습니다.' });
        }
    }, [txtExt, txtCount]);
    ...
    const handleSave = useCallback(async () => {
        if (checkedIds.size === 0) {
            await Swal.fire({ icon: 'warning', text: '선택된 LoginId가 없습니다.' });
            return;
        }
        ...
        try {
            ...
            await Swal.fire({ icon: 'error', title: '오류', text: e?.message || '처리 중 문제가 발생했습니다.' });
        }
...
export default AgentPbxSearchModal;
