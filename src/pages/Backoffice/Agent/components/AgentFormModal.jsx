...
const AgentFormModal = ({ open, onClose, loginId, onSuccess }) => {
    ...
    const handleAutoGenerate = useCallback(async () => {
        if (!insttCode) {
            await Swal.fire({ icon: 'warning', text: 'Agent를 선택해 주세요' });
            return;
        }
        try {
            ...
            if (json?.STATUS === 'SUCCESS') {
                ...
                await Swal.fire({ icon: 'success', text: json?.MESSAGE || '자동 생성 성공' });
            } else {
                setForm(prev => ({ ...prev, idCheck: 'N' }));
                await Swal.fire({ icon: 'error', text: 'Agent 생성 수량 초과' });
            }
        } catch (e) {
    ...
    const handleManualCheck = useCallback(async () => {
        if (!form.loginId) {
            await Swal.fire({ icon: 'warning', text: 'Agent 번호를 입력해 주세요' });
            return;
        }
        try {
            const res = await fnAjaxFetch({
                                        ...
                                        <div className="input-box">
                                            ...
                                            {isEdt ? (
                                                ...
                                                <div className="input-group">
                                                    ...
                                                    >
                                                        {idSetupMode === null ? '번호 선택'
                                                            : idSetupMode === 'auto' ? '자동 생성'
                                                            : '중복 확인'}
                                                    </button>
                                                </div>
                                            )}
